...
Everything that was the owner's own process moved to `CLAUDE.local.md`, git-ignored and loaded by Claude Code beside `AGENTS.md`: the working documents under `.tmp/`, the engineering instruction and the skills it points to, the task protocol with the review pause, marathon mode, the smoke-check rule, Lavish, the modern-stack rule, the skill-writing rules. Codex and Pi read only `AGENTS.md`, which is right for them: they get the public protocol, and the owner works here with Claude Code.

## Acceptance

No path under `.tmp/` or the home directory is named in `AGENTS.md`. Every command in the file was run from a fresh clone with the new file in place. The file reads as documentation: it tells the reader what is true and where each rule is owned, and it dates nothing.

## Smoke check

Sonnet 5, once, from two fresh clones with the new file in place: every command ran (the gate, the docs check, the notebook reads, both regenerations without a diff, `cargo install`), every layout row and reading-order claim held, no owner-private content remained, and `CLAUDE.local.md` carried everything the old file had that the owner still needs. Two must-fix, both taken: the comment rule and the text rule were attributed to pages that did not state them; the development page now states both (a comment is a fact a stranger can check; the words, sentence-case headings, no dash connectives) and the table points there. One should-fix taken: "one Task is in flight at a time" read as a tool invariant and is the skill's practice; the sentence now says so. Two nits taken: the development page now names both docs-check failure modes, and the old "never `/tmp`" rule, which had fallen through both files, is in the owner's local file.
