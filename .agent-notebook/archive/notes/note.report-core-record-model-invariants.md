...
Status: awaiting the owner's review. Working tree only — nothing staged, nothing committed. Local gate green (`./scripts/check.sh`: fmt, clippy `-D warnings` with pedantic, 112 tests, doctests).
...
## The independent review (Opus 5, per the new protocol)
...
## Open questions for the owner (not fixed blindly)
...
`AGENTS.md` — the owner-ordered protocol change (2026-08-27): the review phase is never performed by the authoring model; a separate Opus 5 agent reviews every coding task. Stage 3 rewritten; the Conventions bullet no longer duplicates it.
