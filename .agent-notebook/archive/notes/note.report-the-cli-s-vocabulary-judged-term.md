...
Report for task.the-cli-s-vocabulary-judged-term-by-term. The owner asked for serious analytical work: every word the CLI speaks, at every level, judged on stated criteria, with the reason and the rejected alternatives per verdict. Two words were refused before this report existed — `drop` (the Question's reasoned ending, and the Task ending's first draft) and `withdraw` (the second draft) — which set the bar: no word chosen on the fly.
...
4. **The owner's ear and eye.** Rounded, smooth words without ragged letters. `agent-notebook` is the model; `withdraw` the counterexample.
...
- Inventory first: every verb, flag, state, key, kind, code and reply label at HEAD (`.tmp/docs/research/17-cli-vocabulary-inventory.md`).
- A systematic sweep on Sonnet 5, per the research method: 19 practitioner tools (git, gh, GitHub Issues and Discussions, GitLab, Jira, Linear, Shortcut, Azure Boards, Taskwarrior, todo.txt, beads, tasks-axi, Backlog.md, Gerrit, Phabricator, Bugzilla, Stack Overflow, adr-tools, MADR, PEP 1, IETF) and three theory areas — CLI design guidelines, LLM tool-naming ablations, sound symbolism — every quote verified against its page (`.tmp/docs/research/18-cli-vocabulary-sweep.md`).
- Verdicts here, composed against the four criteria with the sweep as evidence.

### What the sweep says that changes verdicts

- **Near-universal words** exist for a handful of concepts and are kept where we already use them: `reopen` (the most consistent word in the whole sweep), `close`, `blocks / blocked by`, `ready` (every graph-based agent tool: beads, tasks-axi, Taskwarrior), `archive`, `delete`, `edit`, `search`, `status`, `comment`, `label`/`tag` (split down the middle).
...
| `start` | keep; now also `review → active` | The one verb every tool that has the concept uses (Taskwarrior, tasks-axi; Jira "Start progress"). Taking work back from review is starting it again; a separate verb for that exists in no tool in the sweep. | `begin` (rounder, used nowhere); `claim` (beads — ownership, not state). |
...
| `close` | keep; gains `--reason` and `--resolved-by` | Universal (gh, GitLab, beads). It now ends Tasks and Questions alike — one verb for one action across objects. | `done` (Taskwarrior, tasks-axi): equally plain, but `close` already pairs with `reopen` and with the state `closed`. |
...
| `view` | rename → `show` | `git show`, `bd show`, tasks-axi `show`, Bugzilla "Show Bug"; gh's `view` is the minority. One ragged letter instead of two. | `get` (REST vocabulary, not a CLI verb), `info` (Taskwarrior). |
...
| `expunge` | rename → `delete` | Universal (gh, beads, Jira, todo.txt); "expunge" is records-and-law vocabulary. The verb's safeguard — it refuses while anything cites the record — lives in its help line and its refusal, not in an unusual word. | `remove`/`rm` (git's word for tracked files, tasks-axi's), `purge` (Taskwarrior: the second stage after a soft delete — we have no soft stage). |
...
| `--resolved-by <id>` | **new on `close`** (Question) | The record a Question closed into. "Resolves" is what GitLab, Linear and GitHub say when a change settles an item; `-by` with a record value follows `superseded-by` and `blocked-by`. | `--to` (says nothing about the relation), `--answer` (apt as a noun, but the owner's reservation and `resolved-by` covers it), `--routed-to` (network vocabulary). |
| `--drop` (answer) | fold into `close --reason` | — | — |
| `--to` (answer) | fold into `close --resolved-by` | — | — |
| `--note`, `--pr`, `--sha`, `--report`, `--no-proof` | keep | tasks-axi lineage; each names the kind of evidence. | — |
...
| Term | Verdict | Why |
|---|---|---|
| `in-flight:` | rename → `active:` | The state word itself; "in flight" is tasks-axi's aviation metaphor. |
...
## 5. Pivotal choices the owner should glance at first
...
Core (state machine, envelope table, close/resolve, findings, Status labels) → CLI (commands, flags, dispatch, recovery shapes, renderers) → tests renamed and extended → migration script and this notebook → CONTEXT.md, AGENTS.md command names → the Task ending task finishes on the settled word → smoke check → PR.
