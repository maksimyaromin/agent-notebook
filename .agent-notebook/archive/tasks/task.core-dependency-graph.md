...
Progress 2026-08-27: implemented in anb-core, all tests green, independent review pending.
- New `graph.rs` (pub(crate)): TaskGraph over blocked-by edges — is_blocked, path (for would-cycle), cycles (DFS, deterministic, self-loops excluded), unblocked_by (treats the closed id as closed regardless of stored state). Pure model, unit-tested without storage per the spec's testing decision.
...
- Decisions taken (not fixed by spec, flag at review): priority 0 = most urgent, unset ranks as neutral 2; within priority oldest `created` first, id tiebreak — NOTE: the interaction spec worked example shows younger-first within a priority, contradicts oldest-first, ask maintainer. Cycle findings live in check() only (like broken-supersession pairs), NOT in the mutation gate — so `unblock` stays the repair path for a hand-edited cycle; ready excludes cycle members via blockedness, not via invalidity.
...
The independent review round 2026-08-27: 17 findings (3 defects), all fixed except two left deliberately.
...
- Left: ready keeps oldest-first within priority — reviewer independently concludes the interaction spec's worked example (younger-first) is the wrong side and its rows should be re-sorted; maintainer ruling requested. unblock deliberately accepts any well-formed id (not just task.*) — that laxness is the repair path for wrong-typed edges, now stated in its doc.
