...
- 2026-09-12 Maksim Yaromin/codex: Canonical writes now emit flat YAML with quoted scalars and sequences for repeatable fields. Legacy reads and no-op edits preserve bytes; meaningful edits normalize the envelope. Explicit migrate --check reports the normalization work instead of adding a warning to each legacy read. Independent YAML-reader and property tests cover arbitrary single-line titles, scalar typing, Unicode, repeated links and unknown fields. Interrupted migrations preserve recoverable originals and refuse changed files. Verified by the complete code gate and the YAML and transfer suites.
