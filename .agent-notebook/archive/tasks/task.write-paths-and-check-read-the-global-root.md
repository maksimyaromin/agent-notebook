...
- 2026-09-05 Maksim Yaromin: Done 2026-09-05: the user's notebook is a field of the Notebook handle (with_user), read by the write-time nudge, by check for link targets, and by the shadow heuristic from links as from prose; envelope edges stay the project's own. Smoke check (Sonnet 5): two must-fix — a rule cited in prose and linked printed its shadow pair twice (deduped, test added), and the Host field's doc still named Status alone (widened). Report at .tmp/docs/report-global-root-on-every-surface.md; gate green.
