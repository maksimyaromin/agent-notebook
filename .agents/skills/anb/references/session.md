...
```
$ anb add question "Do fences nest?" --from task.grammar-parser-accepts-fences --to Grace
...
questions[1]{id,age,by,to,title}:
  question.do-fences-nest,0d,Ada,Grace,Do fences nest?
budget: ~125 tokens (no ceiling)
```
...
ok: submit task.grammar-parser-accepts-fences — active→review
```

```
$ anb status --by Grace --budget 0
ok: notebook — 4 tasks, 2 decisions, 1 note, 1 question
by: Grace — anb status --team
review[1]{id,taken-by,to}:
  task.grammar-parser-accepts-fences,Ada,Grace
ready[1]{id,priority,age,taken-by,title}:
  task.port-the-parser-to-go,3,0d,Grace,Port the parser to Go
questions[1]{id,age,by,to,title}:
  question.do-fences-nest,0d,Ada,Grace,Do fences nest?
budget: ~113 tokens (no ceiling)
```
...
ok: close task.grammar-parser-accepts-fences — review→closed
...
records[6]{id,state,priority,title}:
