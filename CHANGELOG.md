...
Every version is listed here, newest first, generated by [git-cliff](https://git-cliff.org) from the conventional commits between tags. The Release workflow publishes a tag's entry as the notes of its GitHub release and refuses a tag that has no entry.
