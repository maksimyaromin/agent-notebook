...
/// The emitted map, driven end to end through the real binary: the CLI
/// stays the single write path, and what it writes is one file a reader
/// opens.
mod the_emitted_map {
    use super::*;
    use std::process::Command;

    fn anb(project: &TempDir, line: &[&str]) -> String {
    ...
    /// A map is a derived artifact, so it lands where the caller asked and
    /// never joins the records — a file filed among them would be one every
    /// query had to learn to skip.
    #[test]
    fn a_map_lands_where_it_was_asked_for_and_never_among_the_records() {
        let project = a_project();
        anb(&project, &["add", "Mapped", "--id", "task.mapped"]);

        let reply = anb(&project, &["graph", "--out", "map.html"]);
        let emitted = project.path().join("map.html");

        assert!(emitted.is_file(), "{reply}");
        assert!(reply.contains("map.html"), "the reply says where: {reply}");
        assert_eq!(
            fs::read_dir(project.path().join(".agent-notebook/tasks"))
                .unwrap()
                .count(),
            1,
            "a map is not filed among the records"
        );
    }

    /// A reader opens the file on a desk with no notebook, no tool and no
    /// network beside it, so nothing the page names lives outside it.
    #[test]
    fn nothing_the_page_names_lives_outside_it() {
        let project = a_project();
        anb(&project, &["add", "Blocker", "--id", "task.blocker"]);
        anb(&project, &["add", "Waiter", "--id", "task.waiter"]);
        anb(&project, &["block", "task.waiter", "task.blocker"]);
        anb(&project, &["graph", "--out", "map.html"]);

        let page = fs::read_to_string(project.path().join("map.html")).unwrap();
        assert!(
            page.contains("<polyline"),
            "the fixture draws a line, so the page's own references are in it: {page}"
        );

        for named in references_of(&page) {
            assert!(
                named.starts_with('#'),
                "the page names `{named}`, which is not inside it"
            );
        }
    }

    /// Everything the page points at, whether the browser fetches it before
    /// a script runs or the moment one does: a same-document reference
    /// opens with `#`, and every other spelling names somewhere else.
    fn references_of(page: &str) -> Vec<String> {
        let mut named = Vec::new();
        for (opening, closing) in [("src=\"", '"'), ("href=\"", '"'), ("url(", ')')] {
            let mut rest = page;
            while let Some(at) = rest.find(opening) {
                rest = &rest[at + opening.len()..];
                let end = rest.find(closing).expect("a reference is closed");
    ...
    /// A tile a reader can point at is an element of its own carrying its
    /// id; a picture painted onto one surface is a picture nobody can
    /// anchor a remark to.
    #[test]
    fn every_task_is_its_own_element_and_nothing_is_painted_on_a_canvas() {
        let project = a_project();
        for id in ["task.one", "task.two", "task.three"] {
            anb(&project, &["add", "A demo record", "--id", id]);
        }
        anb(&project, &["graph", "--out", "map.html"]);

        let page = fs::read_to_string(project.path().join("map.html")).unwrap();

        assert_eq!(page.matches("data-id=").count(), 3, "{page}");
        assert!(!page.contains("<canvas"), "{page}");
    }

    /// Every path this tool reports is text, and a path it cannot spell it
    /// could only report as some other file. So it takes none.
    #[cfg(unix)]
    #[test]
    fn a_path_outside_utf8_is_refused_under_the_flag_it_arrived_on() {
        use std::ffi::OsString;
        use std::os::unix::ffi::OsStringExt as _;

        let project = a_project();
        anb(&project, &["add", "Mapped"]);
        let unspellable = OsString::from_vec(b"map\xff.html".to_vec());

        let refused = Command::new(env!("CARGO_BIN_EXE_anb"))
            .args([
                OsStr::new("--json"),
                OsStr::new("graph"),
                OsStr::new("--out"),
        ...
        assert!(
            ...
            "`--out` took a path anb cannot name"
        );
        let payload = String::from_utf8(refused.stderr).unwrap();
        let parsed: serde_json::Value =
            serde_json::from_str(payload.trim()).unwrap_or_else(|_| panic!("not JSON: {payload}"));
        ...
        assert!(
            message.split_whitespace().any(|word| word == "--out"),
            "the refusal says which flag to retype: {message}"
        );

        let leftovers: Vec<String> = fs::read_dir(project.path())
            .unwrap()
            .map(|entry| entry.unwrap().file_name().to_string_lossy().into_owned())
            .filter(|name| name != ".git" && name != ".agent-notebook")
            .collect();
        assert!(
            leftovers.is_empty(),
            "a refused path leaves nothing behind: {leftovers:?}"
        );
    }

    /// A directory that is not there yet is a place the map is put, not a
    /// refusal: the caller named where they want to open the file from.
    #[test]
    fn a_map_makes_the_directory_it_was_asked_for() {
        let project = a_project();
        anb(&project, &["add", "Mapped"]);

        anb(&project, &["graph", "--out", "maps/today/map.html"]);

        assert!(project.path().join("maps/today/map.html").is_file());
    }

    /// A reader opens the file, so a path that cannot hold one is a refused
    /// argument and not a half-written map.
    #[test]
    fn a_path_that_can_hold_no_file_is_refused() {
        let project = a_project();
        anb(&project, &["add", "Mapped"]);
        fs::write(project.path().join("occupied"), "a file, not a directory").unwrap();

        let refused = Command::new(env!("CARGO_BIN_EXE_anb"))
            .args(["--json", "graph", "--out", "occupied/map.html"])
            .current_dir(project.path())
            .env("HOME", project.path())
            .env_remove("ANB_NOTEBOOK")
            .output()
